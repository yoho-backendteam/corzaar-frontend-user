...
interface AddPopupProps {
  isOpen: boolean;
  onClose: () => void;
  onAddCourse: (courseData: CourseFormData) => void;
}

export default function AddPopup({ isOpen, onClose, onAddCourse }: AddPopupProps) {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    if (name.startsWith('pricing.')) {
      ...
      setFormData(prev => ({
        ...
        pricing: {
          ...prev.pricing,
          [pricingField]: pricingField.includes('price') ? parseFloat(value) || 0 : value
        }
      }));
      ...
      setFormData(prev => ({
        ...
        content: {
          ...prev.content,
          [contentField]: parseFloat(value) || 0
        }
      }));
    } else {
      setFormData(prev => ({
  ...
  const addTag = () => {
    if (currentTag.trim() && !formData.category.tags.includes(currentTag.trim())) {
      setFormData(prev => ({
        ...
        category: {
          ...prev.category,
          tags: [...prev.category.tags, currentTag.trim()]
        }
      }));
      setCurrentTag("");
    }
  ...
  const removeTag = (tagToRemove: string) => {
    setFormData(prev => ({
      ...
      category: {
        ...prev.category,
        tags: prev.category.tags.filter(tag => tag !== tagToRemove)
      }
    }));
  };
  ...
  const addSecondary = () => {
    if (currentSecondary.trim() && !formData.category.secondary.includes(currentSecondary.trim())) {
      setFormData(prev => ({
        ...
        category: {
          ...prev.category,
          secondary: [...prev.category.secondary, currentSecondary.trim()]
        }
      }));
      setCurrentSecondary("");
    }
  ...
  const removeSecondary = (secondaryToRemove: string) => {
    setFormData(prev => ({
      ...prev,
      category: {
        ...prev.category,
        secondary: prev.category.secondary.filter(sec => sec !== secondaryToRemove)
      }
    }));
  };
