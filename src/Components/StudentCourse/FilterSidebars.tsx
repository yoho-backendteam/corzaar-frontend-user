...
interface FilterSidebarProps {
  ...
  onModeFilter?: (modes: string[]) => void;
  onLocationFilter?: (radius: number) => void;
...
export default function FilterSidebar({
  ...
  const handleModeChange = (mode: string, isChecked: boolean) => {
    ...
    onModeFilter?.(selectedModes);
  };

  const handleLocationChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    locationRadius = parseInt(event.target.value);
    onLocationFilter?.(locationRadius);
  };
  ...
  const handleResetFilters = () => {
    ...
    sliders.forEach((slider) => {
      (slider as HTMLInputElement).value = (slider as HTMLInputElement).max;
    });
