    ...
    <div className="bg-yellow-400 min-h-screen px-4 sm:px-6 md:px-10 py-6">
      ...
      >
        {["all", "unread", "read"].map((f) => (
          <button
            ...
            className={`px-4 py-2 rounded-md text-sm font-medium capitalize transition-all ${filterType === f
      ...
      <div className="space-y-4">
        {notifications.map((n: any) => (
          <div
            key={n._id}
            className={`bg-white p-4 rounded-md shadow border-l-4 border-t border-b flex flex-col sm:flex-row justify-between sm:items-center gap-3 sm:gap-4 ${n.type === "student"
                ? "border-[#9810FA]"
                : n.type === "merchant"
                  ? "border-[#00A63E]"
                  : "border-[#155DFC]"
